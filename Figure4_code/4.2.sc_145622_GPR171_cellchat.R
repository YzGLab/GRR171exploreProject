...
table(sce$group)
sce$group<-ifelse(sce@assays$RNA@counts['GPR171',]>0,1,0)
sce$group<-paste0(sce$assign.ident,"_",sce$group)



meta<-sce@meta.data
meta$group[meta$assign.ident!='CD8T']=meta$assign.ident[meta$assign.ident!='CD8T']
colnames(meta)
table(meta$group)
...
save(meta,file="CRC_meta_sce.Rdata")###
saveRDS(sce,file="CRC_subsce.RDS")


data.input<-NULL
RNA.res_Zhangzeming_CancerCell_LM<-NULL
cellchat<-NULL

load("CRC_meta_sce.Rdata")
sce<-readRDS("CRC_subsce.RDS")
...
saveRDS(cellchat,file="Figure2/Cellchat/cellchat_LM_GPR171.rds")##first 

#####################
cellchat<-readRDS("Figure2/Cellchat/cellchat_LM_GPR171.rds")
##
##
df.net <- subsetCommunication(cellchat)
write.csv(df.net,file="Figure3/df.net_all_LM.csv")
...
mat<-mat[!colnames(mat) %in% c("Fibroblasts","Myofibroblasts","TMKI67"),!rownames(mat) %in% c("Fibroblasts","Myofibroblasts","TMKI67")]

mat<-mat[!colnames(mat) %in% c("NK","Plasma"),!rownames(mat) %in% c("NK","Plasma")]



groupSize <- as.numeric(table(cellchat@idents[!cellchat@idents %in% c("Fibroblasts","Myofibroblasts","TMKI67")]))

groupSize <- as.numeric(table(cellchat@idents[!cellchat@idents %in% c("Fibroblasts","Myofibroblasts","TMKI67","NK","Plasma")]))
...
pathways.show <- c("SEMA4") # Hierarchy plot# 
...
par(mfrow=c(2,1))
netVisual_aggregate(cellchat, signaling = pathways.show, layout = "circle",label.edge= T, group=group.cellType)
### Chord diagram
par(mfrow=c(1,1))
